...
internal object FoodDataset {
    // Keep large datasets out of <clinit> to avoid "Method too large" compiler errors.
    val defaults: List<FoodSeed> by lazy(LazyThreadSafetyMode.NONE) { buildDefaults() }

    private fun buildDefaults(): List<FoodSeed> = buildList {
        addAll(coreProteinSeeds())
        addAll(grainAndProduceSeeds())
        addAll(legumeAndFatSeeds())
    }

    // Keep each seed group split into multiple small methods so bytecode generation
    // stays below JVM method size limits when the dataset grows over time.

    private fun coreProteinSeeds(): List<FoodSeed> = buildList {
        addAll(coreProteinSeedsPart1())
        addAll(coreProteinSeedsPart2())
    }

    private fun coreProteinSeedsPart1(): List<FoodSeed> = listOf(
    ...
    private fun coreProteinSeedsPart2(): List<FoodSeed> = listOf(
    ...
    private fun grainAndProduceSeeds(): List<FoodSeed> = buildList {
        addAll(grainAndProduceSeedsPart1())
        addAll(grainAndProduceSeedsPart2())
    }

    private fun grainAndProduceSeedsPart1(): List<FoodSeed> = listOf(
    ...
    private fun grainAndProduceSeedsPart2(): List<FoodSeed> = listOf(
    ...
    private fun legumeAndFatSeeds(): List<FoodSeed> = buildList {
        addAll(legumeAndFatSeedsPart1())
        addAll(legumeAndFatSeedsPart2())
    }

    private fun legumeAndFatSeedsPart1(): List<FoodSeed> = listOf(
    ...
    private fun legumeAndFatSeedsPart2(): List<FoodSeed> = listOf(
