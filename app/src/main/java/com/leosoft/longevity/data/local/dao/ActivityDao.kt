...
interface ActivityDao {
    ...
    fun observeStepsRange(startDate: LocalDate, endDate: LocalDate): Flow<List<StepsLogEntity>>

    @Query("SELECT COALESCE(SUM(steps), 0) FROM steps_logs WHERE date BETWEEN :startDate AND :endDate")
    fun observeTotalStepsRange(startDate: LocalDate, endDate: LocalDate): Flow<Int>
